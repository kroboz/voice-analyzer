          ...
          for (var n in tones) {
            ...
            tones[n].forEach(function(t) {
              list += `<div class="tone-select" data-tone-id="${t.tone_id}">
                ${t.tone_name}: ${(t.score * 100).toFixed(0)}%</div>`;
            });
...
Analyzer.prototype.updateStylometry = function(model) {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      //var posPct = (numPos / data.predictions.length * 100.).toFixed(0);
      var posPct = (totalPos / data.predictions.length * 100).toFixed(0);
...
Analyzer.prototype.report = function() {
  ...
  if (this.data.letters)
  {
    $("#word-length").show().html(`<span class='num'>
      Word length: ${
      Math.round(this.data.letters / this.data.words)
    }</span>`);
    $("#sentence-length").show().html(`<span class='num'>
      Sentence length: ${
      Math.round(this.data.words / this.data.sentences)
    }</span>`);
    var puncReport = '';
